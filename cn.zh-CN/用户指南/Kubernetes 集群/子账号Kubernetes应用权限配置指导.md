...
在通过集群访问凭证ConfigKube时，阿里云Kubernetes服务支持字账号Kubernetes应用权限配置，实现子账号应用操作权限的细粒度控制。
...
-   你已成功在集群中部署一个Pod，本例中创建一个名为hello-world的示例Pod。

## 获取子账号KubeConfig {#section_ot2_4v4_f2b .section}

**Note:** 您需要以子账号登录容器服务管理控制台，获取集群访问凭证KubeConfig。
...
**Note:** 若通过kubectl连接集群的方式部署角色模型，您需要以主账号的KubeConfig作为登录凭证，否则无法创建集群访问角色，您将需要复制两次KubeConfig内容。建议通过容器服务控制台或SSH连接到集群的方式进行部署。
...
**Note:** 角色绑定中的subject kind为User，name是子账号对应的UID。
...
1.  SSH登录到Master节点，执行`vim restricted-clusterrole.yaml`命令，创建restricted-clusterrole.yaml文件。
    ...
    rules:
    - apiGroups: [""]
      resources:  ["pods", "pods/attach", "pods/exec", "pods/portforward", "pods/p               ##不允许删除Podroxy"]
      verbs: ["get", "list", “watch"]
    - apiGroups: [""]
      resources:  ["configmaps", "endpoints", "persistentvolumeclaims", "replicationcontrollers", "replicationcontrollers/scale", "secrets", "serviceaccounts", "services", "services/proxy"]
      verbs: ["get", "list", “watch"]
...
3.  ClusterRole和ClusterRolebinding创建完毕后，返回通过子账号ConfigKube凭证连接集群的本机环境。尝试通过kubectl delete命令删除一个Pod。
    ...
    **Note:** 本例中，受限人员的ClusterRole模板不允许被绑定该角色的子账号删除Pod。
